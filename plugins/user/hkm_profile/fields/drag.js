...
var ZDrag = {
	...
	onMouseDown : function(event) {
		...
		if(target.nodeName=="SELECT" || target.nodeName=="INPUT" || target.nodeName=="TEXTAREA" || target.nodeName=="SPAN" || $(target).getParent('.ze_fieldtype_alias_div')) return;
...
function onZDragEndFunction(li){
	...
	var lis=[];
	$$('#userinfo_tab_ul_userinfo .jform_params_userinfofieldName').each(function(el, i){
		if(i)lis[i-1]=el.value;
	});
	$('jform_params_userinfoordering').value=lis.join('|');
}
